...
// index.ts
var Json_GPT_exports = {};
__export(Json_GPT_exports, {
  solve: () => solve,
  solveChat: () => solveChat,
  solveJson: () => solveJson
});
module.exports = __toCommonJS(Json_GPT_exports);

// solve/solve.ts
var import_dotenv = __toESM(require("dotenv"));
var import_openai = require("openai");
import_dotenv.default.config();
var openai = new import_openai.OpenAIApi(new import_openai.Configuration({
...
function formatError(status, error, text) {
  return {
    status,
    data: `{"error": "${error}", "text": "${text}" }`
  };
}
function callGPT(request) {
  return __async(this, null, function* () {
    return (yield openai.createChatCompletion(__spreadValues({
      model: "gpt-3.5-turbo",
      temperature: 0,
      top_p: 1,
      frequency_penalty: 0,
...
var MAX_RETRIES = 4;
var INITIAL_DELAY = 4e3;
var DELAY_EXPONENTIAL = 2;
function solve(request, options) {
  return __async(this, null, function* () {
    const _a = options || {}, {
...
function handleParseError(text, error, z, verbose, safeKey) {
  return __async(this, null, function* () {
    if (verbose)
      console.log("PARSE ERROR", text, error);
    const solved = yield solveParseError(text, error, safeKey);
    if (verbose)
    ...
    if (solved.status === 200)
      try {
        ...
        return handleError(1, "ERROR PARSING HANDLED PARSE ERROR", error2.message, verbose);
      }
    return handleError(solved.status, "ERROR SOLVING HANDLED PARSE ERROR", solved.data, verbose);
  });
}
function handleError(status, error, message, verbose) {
  if (verbose)
...
function fullParse(text, z, verbose, safeKey) {
  try {
...
function zodParse(obj, z, verbose) {
...
// solve/solve-chat.ts
function solveChat(json, options) {
  return __async(this, null, function* () {
    const verbose = (options == null ? void 0 : options.verbose) || false;
    const jsonSchema = getJsonSchema(json.zodSchema, "Output");
