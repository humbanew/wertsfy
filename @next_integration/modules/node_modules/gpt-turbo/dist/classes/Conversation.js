...
export class Conversation {
     ...
     */
    constructor(options) {
        const { id = uuid(), config, requestOptions, history, callableFunctions, plugins = [], pluginsData, } = options ?? {};
        this.id = id;
        ...
        this.requestOptions = new ConversationRequestOptions(this.pluginService, requestOptions);
        this.history = new ConversationHistory(this.pluginService, this.config, history);
        this.callableFunctions = new ConversationCallableFunctions(this.pluginService, callableFunctions);
        this.plugins = new ConversationPlugins(this.pluginService);
        this.chatCompletionService = new ChatCompletionService(this.pluginService, this.config, this.requestOptions, this.history, this.callableFunctions);
    ...
    toJSON() {
        ...
        return conversationSchema.parse(this.pluginService.transformConversationJson(json));
    }
    /**
     ...
     */
    async prompt(prompt, options, requestOptions) {
        const userMessage = this.history.addUserMessage(prompt);
        try {
            await this.pluginService.onUserPrompt(userMessage);
            await this.chatCompletionService.moderateMessage(userMessage, requestOptions);
            const assistantMessage = await this.chatCompletionService.getAssistantResponse(options, requestOptions);
     ...
     * ```
     */
    async reprompt(fromMessageOrId, newPrompt, options, requestOptions) {
        // Find the message to reprompt from
        const id = typeof fromMessageOrId === "string"
            ? fromMessageOrId
            : fromMessageOrId.id;
        ...
        if (previousUserMessage.role !== "user") {
            throw new Error(`Could not find a previous user message to reprompt from (${id}).`);
        }
     ...
     * @returns The assistant's response as a [`Message`](./Message.js) instance.
     */
    async functionPrompt(name, result, options, requestOptions) {
        const transformedResult = await this.pluginService.transformFunctionResult(result);
        const functionMessage = this.history.addFunctionMessage(typeof transformedResult === "string"
            ? transformedResult
            : JSON.stringify(transformedResult), name);
        try {
            await this.pluginService.onFunctionPrompt(functionMessage);
            await this.chatCompletionService.moderateMessage(functionMessage, requestOptions);
            const assistantMessage = await this.chatCompletionService.getAssistantResponse(options, requestOptions);
            return assistantMessage;
        }
        catch (e) {
     ...
     */
    async getChatCompletionResponse(...args) {
