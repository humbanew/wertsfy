import { DEFAULT_CONTEXT, DEFAULT_DISABLEMODERATION, DEFAULT_DRY, DEFAULT_MODEL, DEFAULT_STREAM, } from "../config/constants.js";
import { conversationConfigSchema, } from "../schemas/conversationConfig.schema.js";
/**
...
export class ConversationConfig {
    ...
    _context;
    _dry;
    constructor(pluginService, options = {}) {
    ...
    toJSON() {
        ...
        return conversationConfigSchema.parse(this.pluginService.transformConversationConfigJson(json));
    }
    /**
     ...
     */
    setConfig(config, merge = false) {
        const { context = DEFAULT_CONTEXT, dry = DEFAULT_DRY, disableModeration = DEFAULT_DISABLEMODERATION, ...chatCompletionConfig } = merge ? { ...this.getConfig(), ...config } : config;
        const { apiKey = "", model = DEFAULT_MODEL, stream = DEFAULT_STREAM, frequency_penalty, presence_penalty, max_tokens, logit_bias, stop, temperature, top_p, user, function_call, } = chatCompletionConfig;
        this.apiKey = apiKey;
        this.dry = dry;
        this.model = model;
    ...
    set dry(dry) {
        this._dry = dry;
        if (!dry && !this.apiKey) {
            console.warn("[gpt-turbo] No OpenAI API key was provided. Conversation will run on dry mode. If this was intentional, you should explicitly set the 'dry' parameter to 'true'.");
            this._dry = true;
        }
    }
    get isModerationEnabled() {
        return this.isModerationStrict || this.isModerationSoft;
    }
    get isModerationStrict() {
        if (!this.apiKey)
            return false;
        return !this.disableModeration;
    }
    get isModerationSoft() {
        if (!this.apiKey)
            return false;
    ...
    set stop(stop) {
        if (Array.isArray(stop) && !stop.length) {
            console.warn("[gpt-turbo] The 'stop' parameter was provided as an empty array, which would cause a Bad Request error from OpenAI. The 'stop' parameter will be set to 'null' instead.");
            this._stop = null;
            return;
        }
        if (typeof stop === "string" && !stop.trim()) {
            console.warn("[gpt-turbo] The 'stop' parameter was provided as an empty string, which would cause a Bad Request error from OpenAI. The 'stop' parameter will be set to 'null' instead.");
            this._stop = null;
            return;
        }
        this._stop = stop;
