...
import { conversationHistorySchema, } from "../schemas/conversationHistory.schema.js";
...
export class ConversationHistory {
    ...
    config;
    messages = [];
    addMessageEvents = new EventManager();
    removeMessageEvents = new EventManager();
    constructor(pluginService, config, options = {}) {
        ...
        for (const message of messages) {
            switch (message.role) {
                ...
                case "function":
                    ...
                    if (message.content === null)
                        throw new Error("Function message content cannot be null.");
    ...
    toJSON() {
        ...
        return conversationHistorySchema.parse(this.pluginService.transformConversationHistoryJson(json));
    }
    /**
    ...
    addMessage(message) {
        ...
        if (message.role === "system") {
            ...
            if (this.messages[this.messages.length - 1]?.role === message.role) {
    ...
    addAssistantMessage(message) {
        const assistantMessage = new Message("assistant", message, this.config.model);
    ...
    addFunctionCallMessage(functionCall) {
        const functionCallMessage = new Message("assistant", null, this.config.model);
     ...
     * @returns The [Message](./utils/types.ts) object that was added to the conversation
     */
    addFunctionMessage(message, name) {
        const functionMessage = new Message("function", message, this.config.model);
     ...
     */
    getMessages(includeContext = false) {
        if (!this.getContext())
     ...
     */
    getCreateChatCompletionMessages() {
